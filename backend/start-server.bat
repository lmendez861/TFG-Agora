...
set "APP_SERVER_LAN_HOST="

if /I "%APP_SERVER_BIND_HOST%"=="0.0.0.0" (
    for /f %%i in ('powershell -NoProfile -Command "$route = Get-NetRoute -DestinationPrefix '0.0.0.0/0' -ErrorAction SilentlyContinue | Sort-Object RouteMetric,InterfaceMetric | Select-Object -First 1; if ($route) { Get-NetIPAddress -InterfaceIndex $route.InterfaceIndex -AddressFamily IPv4 -ErrorAction SilentlyContinue | Where-Object { $_.IPAddress -notlike '169.254*' } | Select-Object -First 1 -ExpandProperty IPAddress }"') do set "APP_SERVER_LAN_HOST=%%i"
)
...
if not "%APP_SERVER_LAN_HOST%"=="" echo URL LAN: http://%APP_SERVER_LAN_HOST%:%APP_SERVER_PORT%
echo.
